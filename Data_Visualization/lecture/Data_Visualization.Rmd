...
For different plotting per year - you must make it a factor
```{r geom_box_fac}
...
- `xlab`/`ylab` - functions to change the labels
...
- theme - overall set of parameters of the plot
- see `?theme_bw` - for ggthemes
...
- `theme` - change the overall theme or specific elements
...
```{r hist_death, comment="",prompt=TRUE, fig.align='center', cache=FALSE}
...
```{r gdens_line_alpha_death, comment="",prompt=TRUE, fig.align='center', cache = FALSE}
...
```{r geom_line_qplot_nocol, comment="",prompt=TRUE, fig.align='center', cache=FALSE}
...
```{r ghist_chick, comment="",prompt=TRUE, fig.align='center', cache = FALSE}
qplot(x = weight, 
      fill = factor(Diet),
      data = ChickWeight, 
      geom = c("histogram"))
...
```{r ghist_chick_alpha, comment="",prompt=TRUE, fig.align='center', cache = FALSE}
...
We cold also do densities
```{r gdens_chick, comment="",prompt=TRUE, fig.align='center', cache = FALSE}
qplot(x= weight, fill = Diet, data = ChickWeight, 
      geom = c("density"), alpha=I(.7))
```
...
```{r gdens_chick_alpha, comment="",prompt=TRUE, fig.align='center', cache = FALSE}
